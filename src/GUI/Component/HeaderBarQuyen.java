...
public class HeaderBarQuyen extends javax.swing.JPanel {
    ...
    private void initComponents() {
        ...
        jComboBoxSearchType.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Tên quyền" }));
    ...
    private void jComboBoxSearchTypeActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jComboBoxSearchTypeActionPerformed
        // Hiện tại chưa dùng, có thể mở rộng
    }//GEN-LAST:event_jComboBoxSearchTypeActionPerformed

    private void jButtonSearchActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonSearchActionPerformed
        String keyword = jTextFieldSearchBar.getText().trim().toLowerCase();
        if (keyword.isEmpty()) {
            phanQuyenPanel.loadTableData();
            return;
        }
        List<QuyenDTO> list = phanQuyenPanel.quyenBUS.getListQuyen();
        DefaultTableModel model = (DefaultTableModel) phanQuyenPanel.jTable1.getModel();
        model.setRowCount(0);
        for (QuyenDTO q : list) {
            if (q.getTen().toLowerCase().contains(keyword)) {
                model.addRow(new Object[]{q.getId(), q.getTen()});
            }
        }
    ...
    private void jButtonDetailActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonDetailActionPerformed
        int selectedId = phanQuyenPanel.getSelectedTableId();
        if (selectedId == -1) {
            JOptionPane.showMessageDialog(this, "Vui lòng chọn quyền để xem chi tiết!", "Thông báo", JOptionPane.WARNING_MESSAGE);
            return;
        }
        List<QuyenDTO> list = phanQuyenPanel.quyenBUS.getListQuyen();
        QuyenDTO selectedQuyen = null;
        ...
        if (selectedQuyen != null) {
            JOptionPane.showMessageDialog(this,
                    "ID: " + selectedQuyen.getId() + "\nTên: " + selectedQuyen.getTen(),
                    "Chi tiết quyền",
